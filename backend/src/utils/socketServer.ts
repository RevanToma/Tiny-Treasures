...
export class SocketServer {
  ...
  public start() {
    io.on('connection', async (socket: CustomSocket) => {
      const token: string | undefined = socket.handshake.query?.userId
        ?.toString()
        .trim();
      ...
      socket.on('typing', (typingInfo: TypingInfo) =>
        this.typing(io, socket, typingInfo)
      );
  ...
  private createChat = async (io: Server, socket: CustomSocket, data: any) => {
    ...
    if (userId) {
      ...
      const sendTo: string[] = recvSocketId
        ? [socket.id, recvSocketId]
        : [socket.id];
  ...
  private typing = async (
    ...
    if (recvSocketId) io.to(recvSocketId).emit('typing', typingInfo);
  };
  ...
  private onChatMessage = async (
    ...
    const recvSocketId: string | undefined =
      this.connectedUsers[receiverId.toString()];
